...
print("==== DEBUG: start diamond block ====")

print("Working directory:")
print(getwd())

print("Listing diamond folder content:")
print(list.files(file.path(project_folder, "results", "read_annotate", "diamond")))

diamond_groups <- list.dirs(file.path(project_folder, "results", "read_annotate", "diamond"), recursive = FALSE, full.names = FALSE)
print(diamond_groups)
print(paste("diamond_groups length =", length(diamond_groups)))
...
for(diamond_run in 1:length(diamond_groups)){
  ...
  for(sample_run in 1:nSamples){
    ...
    tmp <- readLines(file.path(file.path(project_folder, "results", "read_annotate", "diamond",
...
for(sample_run in 1:nSamples){
  ...
  tmp <- readLines(file.path(file.path(project_folder, "results", "read_annotate", "humann", "log",
...
```{r}
kraken_groups <- list.dirs(file.path(project_folder, "results", "read_annotate", "kraken2"), recursive = FALSE, full.names = FALSE)
...
for(kraken_run in 1:length(kraken_groups)){
  ...
  tmp <- readLines(file.path(file.path(project_folder, "results", "read_annotate", "kraken2", kraken_groups[kraken_run],
                                       paste0(sample_sheet$sample_id[sample_run], ".", sample_sheet$library_id[sample_run], ".log" ))))
...
for(sample_run in 1:nSamples){
  ...
  tmp <- readLines(file.path(file.path(project_folder, "results", "read_annotate", "metaphlan", "profiled",
...
result_matrix_nonpareil <- read.table(file.path(project_folder, "results", "read_annotate", "nonpareil", "nonpareil.tsv"), sep="\t", header=TRUE)
...
for(sample_run in 1:nrow(sample_sheet)){
  tar_file <- paste0("/scratch/project_2010176/metaG_groupAssembly/results/read_annotate/phyloflash/",
...
for(sample_run in 1:nSamples){
  ...
  tmp <- readLines(file.path(file.path(project_folder, "results", "read_annotate", "singlem", "pipe",
                                       paste0(sample_sheet$sample_id[sample_run], ".", sample_sheet$library_id[sample_run], ".log" ))))
